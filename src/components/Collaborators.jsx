...
const Collaborators = ({ list, listFiltered, setListFiltered, setListCollaborators }) => {
    ...
    const deleteCollaborator = id => {
        if(window.confirm(`¿Seguro que deseas eliminar al Colaborador ${list[list.findIndex(e => e.id === id)].name} cuyo ID es ${id}?`)){
            setListCollaborators(list.filter(item => item.id != id));
            setListFiltered(listFiltered.filter(item => item.id != id));
        }
    }
                        ...
                        listFiltered.map(
                            item => <tr key={item.id}>
                                ...
                                <td valign="middle" className="text-end">
                                    <button onClick={(e) => deleteCollaborator(item.id)} className="btn">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="red" height="24" width="24" viewBox="0 0 48 48">
